...
public class FileUploadController {
    ...
    /*MultipartFile :
    * - 요청 형식이 multipart/from-data 일때
    *  MultipartFile가 구분하여 처리한 데이터중 File만 지칭하는 객체
    * */
    ...
    public String singleFileUpload(
        ...
        System.out.println(singleFile.getSize()); // 제출 파일 크기(byte)
        /* 서버 컴퓨터에 제출된 파일을 저장 */
        ...
        // 2. 서버에 저장할 파일명 변경하기
        // - 같은 폴더 내에는 같은 이름의 파일이 줄복 저장될 수 없다!!
        String savedName = generateSavedFileName(singleFile);


        // 3. 지정된 경호에 변경된 이름으로 파일 저장하기
        // - 파일 저장 전 파일은 어디에 있을까?
        //  기본적으로 는 메모리에 로드되어 있음
    ...
    public String multiFileUpdateDB(
        ...
        try{
            for(MultipartFile singleFile : multiFile){
                ...
                // 지정된 경로에 파일로 저장
                singleFile.transferTo(new File(filePath + "/" + savedName));
                if (files.size() >= 3){ //저장된 파일이 3개다 괴면 임의로 예외발생
            ...
            model.addAttribute("message","파일 업로드싫패");
