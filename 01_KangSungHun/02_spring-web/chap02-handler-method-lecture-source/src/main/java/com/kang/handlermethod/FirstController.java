...
public class FirstController {
    ...
    /* 핸들러 메서드의 반환형이 void이면 요청 주소가 곧 view의 이름이 된다.
     * - 요청 주소 : /first/regist
     ...
     *
     * - ArgumentResolver의 리스트에 작성된 타입이 호출되면 자동으로 생성해서 주입
     * */

    /* 1. WebRequest로 요청 파라미터 전달 받기
     * - HttpServletRequest를 이용해서 파라미터를 얻어올 수 있었음
     * - WebRequest를 이용해 Servlet에 종속되지 않은 Spring 기반 객체를 사용
     * */
    ...
    public String registMenu(Model model, WebRequest req) {
        ...
        String message
                = String.format("%s을/를 신규 메뉴 목록의 %d 카테고리에 %d원으로 등록했습니다.", name, categoryCode, price);
    ...
    public String modifyMenu(Model model,
                             @RequestParam(value = "modifyName", defaultValue = "xxx") String modifyName,
    ...
    *   -> 전달 받은 파라미터를 저장한 객체 == 커맨드 객체라 불린다.
    *
    * - () 내부에 속성명을 지정하면
    ...
    *
    * - 사용법
    * 1) @SessionAttributes({"key1", "key2"})
    * 2) Model.addAddAttributge("key1", "value1")
    *   -> @SessionAttributes에 등록된 "key1" 이라는 key를 이용해서 model의 속성으로 추가하게 되면
    ...
    * - sessionStatus 라는 세션 상태를 관리하는 스프링 객체를 얻어와
      ...
      * @sessionttributes와는 따로 관따로 관리가 되고있ㄸㄸ
      * ->둘을 혼용해서 사용하고 있다면
    ...
    @GetMapping("/logout")
    public String logoutTest(SessionStatus sessionStatus) {
